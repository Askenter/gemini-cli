...
The Torch Pilot requires you to authenticate with Google's AI services. On initial startup you'll need to configure **one** of the following authentication methods:

1.  **Login with Google (Gemini Code Assist):**
    - Use this option to log in with your google account.
    - During initial startup, Torch Pilot will direct you to a webpage for authentication. Once authenticated, your credentials will be cached locally so the web login can be skipped on subsequent runs.
    - Note that the web login must be done in a browser that can communicate with the machine Torch Pilot is being run from. (Specifically, the browser will be redirected to a localhost url that Torch Pilot will be listening on).
...
Torch Pilot automatically loads environment variables from the **first** `.env` file it finds, using the following search order:
...
When running the Torch Pilot in a non-interactive environment, you cannot use the interactive login flow.
