...
- **Q: How do I update Torch Pilot to the latest version?**
  - A: If installed globally via npm, update Torch Pilot using the command `npm install -g @google/gemini-cli@latest`. If run from source, pull the latest changes from the repository and rebuild using `npm run build`.

- **Q: Where are Torch Pilot configuration files stored?**
...
- **Error: Command not found (when attempting to run Torch Pilot).**
  - **Cause:** Torch Pilot is not correctly installed or not in your system's PATH.
  - **Solution:**
    1.  Ensure Torch Pilot installation was successful.
